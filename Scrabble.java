...
public class Scrabble {
	...
	public static boolean isWordInDictionary(String word) {
		// can use the MyString function from HW4, but I assumed you can use it because you use it in "init"
		for (int w = 0; w < DICTIONARY.length; w++) {

			// if equals() is not allowed:
			// if (MyString.subsetOf(word, DICTIONARY[w]) && word.length() == DICTIONARY[w].length()) {
			// 	return true;
			// }

			if (word.equals(DICTIONARY[w])) { 
				return true;
			}
		}
		return false;
	...
	public static int wordScore(String word) {
		...
		for (int l = 0; l < word.length(); l++) {
			char currentChar = word.charAt(l);
			totalScore += SCRABBLE_LETTER_VALUES[currentChar - 97]; 
		}
	...
	public static String createHand() {
		String playerHand = MyString.randomStringOfLetters(HAND_SIZE - 2);
		playerHand = MyString.insertRandomly('a', playerHand);
		playerHand = MyString.insertRandomly('e', playerHand);
		return playerHand;
	}
